...
public:
    ...
    void import(const std::filesystem::path & FilePath, size_t Order, size_t Degree) {
        ...
        std::cout << "INFO  | Start reading gravity_stokes data from file " << FilePath.string() << std::endl;
        ...
        if (file.good()) {
            ...
            _gm = std::stod(line);
                ...
                std::cout << "ERROR | Failed to read GM, " << FilePath.string() << std::endl;
                file.close();
            }
            ...
            if (std::getline(file, line)) {
                ...
                std::cout << "ERROR | Failed to read equatorial radius, " << FilePath.string() << std::endl;
                file.close();
            }
            ...
            if (std::getline(file, line)) {
                ...
                std::cout << "ERROR | Failed to read max order, " << FilePath.string() << std::endl;
                file.close();
                return;
            }
            ...
            if (std::getline(file, line)) {
                ...
                std::cout << "ERROR | Failed to read max degree, " << FilePath.string() << std::endl;
                file.close();
                return;
            }
            ...
            if (max_order < Order) {
                std::cout << "WARN  | Required order is less than the given one. " << max_order << " will be used, " << FilePath.string() << std::endl;
                _order = max_order;
            }
            if (max_degree < Degree) {
                std::cout << "WARN  | Required degree is less than the given one. " << max_degree << " will be used, " << FilePath.string() << std::endl;
                Degree = max_degree;
            }
            if (Order < Degree) {
                std::cout << "WARN  | Order can not be less than degree. " << Order << " will be used as degree value, " << FilePath.string() << std::endl;
                Order = Degree;
            }
            ...
            while (total > 0 && std::getline(file, line)) {
                ...
                if(iss >> n >> m >> c >> s) {
                    ...
                    std::cout << "ERROR | Failed to read C and S data, " << FilePath.string() << std::endl;
                    file.close();
                    return;
                }
            }
            ...
            if (total > 0) {
                std::cout << "WARN  | C and S are missing: " << total << " values. 0 will be used instead of them," << FilePath.string() << std::endl;
            }
            ...
            std::cout << "INFO  | Graivty field loaded successfully from file " << FilePath.string() << std::endl;
            file.close();
        }
        else {
            std::cout << "ERROR | Failed to load gravity data from file " << FilePath.string() << std::endl;
        }
    }
